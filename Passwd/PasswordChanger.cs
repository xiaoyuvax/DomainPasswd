            ...
            using (var context = this.domainContextFactory.Create(domain, options.UseSsl))
            {
                ...
                using (domainUser)
                {
                    domainUser.ChangePassword(
                        options.ConsoleMode ? this.commandLinePasswordChangerSource.OldPassword() : options.OldPassword,
                        options.ConsoleMode ? this.commandLinePasswordChangerSource.NewPassword() : options.NewPassword);
                }
            }
        }
    }
